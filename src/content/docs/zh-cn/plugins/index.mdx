...
StudioCMS 插件类似于 Astro 集成（Integrations），但在函数对象上附加了额外元数据。这些元数据用于确定插件的加载和使用方式。插件通过添加新功能或修改现有行为来扩展 StudioCMS 的能力。
...
此示例定义了一个需要 StudioCMS v0.1.0-beta.18+ 的插件。当触发 `astro:config:setup` 生命周期钩子时，会在控制台打印消息。
