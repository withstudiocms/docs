...
### 自定义API端点
```ts title="/src/pages/api/custom-endpoint.ts"
...
### 扩展仪表板路由
```astro title="/src/pages/dashboard/custom-page.astro"
---
import DashboardLayout from 'studiocms:components/DashboardLayout';
---
...
> **调试提示**：在开发模式下，可通过 `?debug=1` 参数查看虚拟模块加载详情：
> ```
> http://localhost:4321/dashboard/?debug=1
> ```

此架构使 StudioCMS 既能作为开箱即用的解决方案，又能通过其扩展机制满足高度定制化需求。