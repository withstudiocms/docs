...
/**
 * Un servicio de almacenamiento No-Op que implementa la interfaz StorageApiBuilderDefinition.
 *
...
export default class NoOpStorageService<C, R> implements 
  StorageApiBuilderDefinition<C, R> {
    ...
    getPOST(_?: AuthorizationType): StorageAPIEndpointFn<C, R> {
      return this.driver.handleEndpoint(async () => this.#respuestaCompartida());
    }
...
<ReadMore>
Para conocer los plugins de gestión de almacenamiento disponibles, consulta el [Catálogo de paquetes](/es/package-catalog#storage-managers).
</ReadMore>