...
```sh
CMS_LIBSQL_URL=http://localhost:8080 # Esto debería apuntar a tu servidor docker/sistema local
CMS_LIBSQL_AUTH_TOKEN= # pega tu token JWT codificado en base64 aquí
```
...
- Usando el token JWT como variable de entorno (el mismo valor CMS_LIBSQL_AUTH_TOKEN que usaste en tu archivo .env)
...
2. Sincroniza el esquema de tu base de datos StudioCMS:
   ...
   <PackageManagers type="exec" args="studiocms migrate --latest" />
...
[getting-started]: /es/start-here/getting-started/#primera-configuración-o-durante-actualizaciones-si-el-esquema-de-tablas-es-actualizado
