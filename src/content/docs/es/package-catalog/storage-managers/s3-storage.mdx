...
description: "Complemento de gestión de almacenamiento S3 de StudioCMS, proporciona soporte para almacenamiento S3 de AWS para StudioCMS."
---
...
2. Añade este complemento en tu configuración de StudioCMS. (`studiocms.config.mjs`)
...
3. Configura las variables de entorno requeridas en tu archivo `.env`:

   ```sh title=".env"
   CMS_S3_PROVIDER=AWS
   CMS_S3_REGION=us-east-1
   CMS_S3_ENDPOINT=  # Dejar vacío para AWS
...
CMS_S3_BUCKET_NAME=your_bucket_name
CMS_S3_FORCE_PATH_STYLE=true  # Requerido para MinIO
CMS_S3_PUBLIC_ENDPOINT=  # Opcional: Endpoint público personalizado para acceder a los archivos
```
