...
description: "Options for rendering Content on a Custom Frontend"
---
...
<Aside>
Please note that using our rendering system requires you to have a studiocms plugin that provides a renderer, such as `@studiocms/md`, `@studiocms/html` or a [custom plugin that provides a renderer](/en/plugins/renderers/) for your content type. The renderer will handle the rendering of the content based on the pageType defined in your StudioCMS configuration.
</Aside>
...
<ReadMore>
Want to build a custom renderer plugin for StudioCMS? Check out our guide on [Creating Custom Renderer Plugins](/en/plugins/renderers/) to learn how to extend StudioCMS with your own rendering logic.
</ReadMore>