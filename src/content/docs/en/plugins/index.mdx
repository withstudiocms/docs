...
### The StudioCMS Plugin Type <Badge variant="caution" text="Revised 0.4.0" />
...
In this example, we define a StudioCMS plugin called `My Plugin`. The plugin also provides an Astro Integration that logs a message to the console when the `astro:config:setup` hook is called.
