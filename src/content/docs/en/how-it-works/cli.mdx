...
- **[`@studiocms/migrator`](#studiocmsmigrator)**: Migrate your StudioCMS project from older versions to the latest version.
- **[`@studiocms/upgrade`](#studiocmsupgrade)**: Upgrade StudioCMS Projects and their Plugins.
...
COMMANDS
  ...
  - crypto            Crypto Utilities for StudioCMS Security
  ...
  - migrate           Manage database migrations for StudioCMS.
```
...
```log
usage: studiocms debug [options]

Displays debug information about your StudioCMS installation.
...
1. Set up your environment file (.env) with options to:
   ...
   - Use the interactive `.env` builder which can:
     - Set up a database connection (libSQL, MySQL, PostgreSQL)
...
```log
usage: studiocms migrate [options]

Manage database migrations for StudioCMS.

Options:
  -h, --help      display help for command
  --debug         Enable debug mode
  -r, --rollback  Rollback the last migration
  -l, --latest    Migrate to the latest version
...
A command-line tool for migrating your StudioCMS project from older versions to the latest version.
...
<Steps>
1. Install the migration tool into the root of your StudioCMS project:
   <PackageManagers pkg="@studiocms/migrator" type='add' />

2. Once installed, add the following script to your `package.json` file:
...
3. Start the migration tool by running the following command in your project directory:
