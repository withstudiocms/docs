...
```sh
CMS_LIBSQL_URL=http://localhost:8080 # Cela devrait pointer vers votre serveur Docker/système local
CMS_LIBSQL_AUTH_TOKEN= # collez ici votre jeton d’authentification JWT standard
```
...
- En utilisant le jeton JWT comme variable d’environnement (la même valeur `CMS_LIBSQL_AUTH_TOKEN` que celle que vous avez utilisée dans votre fichier `.env`)
